	...
	synchronized public ArrayList<ImageData> executeQuery(SearchBox sb) throws SQLException{
		...
		if(!"".equals(representativeFrame)){
			for(int j=0; j<result.size(); j++){
				...
				if(Integer.parseInt(i.title.substring(0,i.title.length()-4))-1==Integer.parseInt(lastTitle.substring(0,lastTitle.length()-4))){
					currentSeries.add(i);
				}
				else{
...
//			System.out.println(refinedResult.size()+"\t"+currentSeries.size());
		...
		if( (marginSizeBefore!=null && marginSizeBefore>0) || (marginSizeAfter!=null && marginSizeAfter>0) ){
			...
			for(int i=0; i<result.size(); i++){
				...
				if(theImage.segment_id != lastSegmentID){
					...
					addPeripheralImageToResult(refinedResult,videoID, theImage.title,-marginSizeBefore);
				}
		...
		if(marginSizeAfter!=null && marginSizeAfter>0){			
			addPeripheralImageToResult(result,videoID, lastTitle,marginSizeAfter);
		}
	...
	public String getEnvironment(String id) throws SQLException {
		...
		for(int i=0; i<ids.length; i++){
			if(ids[i].equals(String.valueOf(segmentID))){
				if(MainFrame.targetSegmentDisplayMode.isSelected()){
